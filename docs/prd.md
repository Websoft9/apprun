...
以下列表供架构阶段评估参考，**不代表最终选型**：

**认证与权限服务**：
- Ory Kratos - 现代化身份认证系统
- Keycloak - 企业级身份和访问管理
- Auth0 - 商业认证服务
- 自研方案 - 基于 JWT + RBAC
...
**对象存储**：
- MinIO - 开源对象存储
- AWS S3 / 阿里云 OSS / 腾讯云 COS - 云存储
- 自研方案 - 基于文件系统封装

**消息队列/事件总线**：
- NATS - 轻量级消息系统
- Kafka - 高吞吐量消息队列
- Redis Streams - 基于 Redis 的流式数据
- RabbitMQ - 功能丰富的消息队列

**实时推送服务**：
- Centrifugo - 实时消息服务器
- Socket.IO - WebSocket 库
- 自研方案 - 基于 WebSocket 实现

**API 网关**：
- Kong - 云原生 API 网关
- Traefik - 现代化反向代理和负载均衡
- APISIX - 高性能 API 网关
- 自研方案 - 基于 Go/Rust 实现

**日志和监控**：
- Loki + Grafana - 日志聚合和可视化
