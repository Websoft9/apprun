...
└────────┬────────────────────────────────────────┘
         ...
         ▼
┌─────────────────────────────────────────────────┐
│     pkg/metrics/storage (Anti-Corruption)       │
│  - Storage Interface                           │
...
├── storage_handler.go      # Storage API handlers (NEW)
├── collector_test.go       # Unit tests for collector
├── service_test.go         # Unit tests for service
├── storage_handler_test.go # Unit tests for storage (NEW)
└── README.md               # This file
```
...
**Storage Endpoints**:
- `POST /api/observability/metrics/ingest` - Store metrics
- `GET /api/observability/metrics/query` - Query metrics by time range
- `GET /api/observability/metrics/health` - Storage health check
...
r.Post("/api/observability/metrics/ingest", storageHandler.Ingest)
r.Get("/api/observability/metrics/query", storageHandler.Query)
```
...
**HTTP Request**:
```bash
curl -X POST http://localhost:8080/api/observability/metrics/ingest \
...
**By Duration**:
```bash
curl "http://localhost:8080/api/observability/metrics/query?name=api_requests_total&duration=1h"
```

**By Time Range**:
```bash
curl "http://localhost:8080/api/observability/metrics/query?name=api_requests_total&start=2026-01-21T00:00:00Z&end=2026-01-21T12:00:00Z&limit=100"
```
...
```bash
cd core
go test ./modules/obs/... -v -cover
```

### Run Storage Handler Tests
```bash
go test ./modules/obs/... -v -run TestStorageHandler
```

### Run with Race Detection
```bash
go test ./modules/obs/... -race
```
...
- **apprun/pkg/metrics**: Metrics repository layer
