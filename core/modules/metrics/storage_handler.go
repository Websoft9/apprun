...
func NewStorageHandler(repo *metricstore.Repository) *StorageHandler {
	return &StorageHandler{
		repo: repo,
	}
}

// NOTE: IngestRequest moved to types.go to avoid duplication
...
// Ingest handles POST /api/metrics/storage/ingest
...
// @Router /api/metrics/storage/ingest [post]
...
// Query handles GET /api/metrics/storage/query
...
// @Failure 403 {object} response.Response "Forbidden - requires admin role"
// @Failure 500 {object} response.Response "Internal Server Error"
// @Router /api/metrics/storage/query [get]
func (h *StorageHandler) Query(w http.ResponseWriter, r *http.Request) {
	...
	if limitStr != "" {
		parsedLimit, err := strconv.Atoi(limitStr) //nolint:govet  //nolint:govet 
...
// Health handles GET /api/metrics/storage/health
...
// @Router /api/metrics/storage/health [get]
