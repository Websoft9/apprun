...
func (_q *UserQuery) QueryServers() *ServersQuery {
...
func (_q *UserQuery) WithServers(opts ...func(*ServersQuery)) *UserQuery {
	query := (&ServersClient{config: _q.config}).Query()
...
func (_q *UserQuery) loadServers(ctx context.Context, query *ServersQuery, nodes []*User, init func(*User), assign func(*User, *Servers)) error {
	...
	for _, n := range neighbors {
		fk := n.user_servers
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_servers" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
