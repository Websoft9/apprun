...
func (_q *UsersQuery) QueryServers() *ServersQuery {
	query := (&ServersClient{config: _q.config}).Query()
...
func (_q *UsersQuery) WithServers(opts ...func(*ServersQuery)) *UsersQuery {
	query := (&ServersClient{config: _q.config}).Query()
...
func (_q *UsersQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Users, error) {
	...
	if query := _q.withServers; query != nil {
		if err := _q.loadServers(ctx, query, nodes,
			func(n *Users) { n.Edges.Servers = []*Servers{} },
			func(n *Users, e *Servers) { n.Edges.Servers = append(n.Edges.Servers, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UsersQuery) loadServers(ctx context.Context, query *ServersQuery, nodes []*Users, init func(*Users), assign func(*Users, *Servers)) error {
	...
	query.withFKs = true
	query.Where(predicate.Servers(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(users.ServersColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.users_servers
		if fk == nil {
			return fmt.Errorf(`foreign-key "users_servers" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "users_servers" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
