...
type Config struct {
	...
	AccessTokenExpiration time.Duration `yaml:"access_token_expiration" json:"access_token_expiration" default:"24h" db:"true" validate:"min=1h,max=168h"`

	// RefreshTokenExpiration defines how long refresh tokens are valid (for future use).
	// db:"true" - Can be updated dynamically via config center.
	RefreshTokenExpiration time.Duration `yaml:"refresh_token_expiration" json:"refresh_token_expiration" default:"168h" db:"true" validate:"min=24h,max=720h"`

	// Issuer is the JWT "iss" claim value.
	// db:"false" - Infrastructure config, not changeable at runtime.
	Issuer string `yaml:"issuer" json:"issuer" default:"apprun-platform" db:"false" validate:"required"`

	// Audience is the JWT "aud" claim value (optional, for token validation).
	// db:"false" - Infrastructure config, not changeable at runtime.
	Audience string `yaml:"audience" json:"audience" default:"apprun-api" db:"false" validate:"required"`
	...
	WhitelistPaths []string `yaml:"whitelist_paths" json:"whitelist_paths" db:"true" validate:"min=1,dive,uri"`
}
...
type LegacyConfig struct {
	...
	Expiry         string   `yaml:"expiry" json:"expiry"` // Duration string like "24h"
	Issuer         string   `yaml:"issuer" json:"issuer"`
	WhitelistPaths []string `yaml:"whitelist_paths" json:"whitelist_paths"`
}
