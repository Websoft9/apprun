...
//	defer repo.Close()
func NewRepository(storage storage.Storage, cfg *Config) *Repository {
	return &Repository{
		storage: storage,
		cfg:     cfg,
		logger:  logger.L(),
...
func (r *Repository) executeWithRetry(ctx context.Context, fn func() error) error {
	...
	for attempt := 1; attempt <= maxAttempts; attempt++ {
		...
		select {
		case <-ctx.Done():
			return storage.ErrTimeout("operation", "context cancelled")
		default:
		}
		...
		if attempt < maxAttempts {
			...
			case <-ctx.Done():
				timer.Stop()
				return storage.ErrTimeout("operation", "context cancelled during backoff")
			}
		}
	}
