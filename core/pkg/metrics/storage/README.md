...
```bash
cd core
go test ./pkg/metrics/storage/... -v -race -cover
```

Run with coverage report:

```bash
go test ./pkg/metrics/storage/... -coverprofile=coverage.out
...
```bash
go test ./pkg/metrics/storage/... -bench=. -benchmem
```
...
- `apprun/pkg/metrics`: Repository layer with retry logic
