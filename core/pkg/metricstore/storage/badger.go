...
var (
	...
	diskUsageBytes metric.Int64ObservableGauge //nolint:unused // Used in init() for metrics registration
...
func (s *BadgerStorage) Store(ctx context.Context, metric Metric) error { //nolint:gocritic // Parameter name is clear
	start := time.Now()
	defer func() {
...
func (s *BadgerStorage) Query(ctx context.Context, name string, start, end time.Time, limit int) ([]Metric, error) {
	...
	err := s.db.View(func(txn *badger.Txn) error {
		...
		for it.Seek(startKey); it.Valid() && len(metrics) < limit; it.Next() {
			...
			select {
			case <-ctx.Done():
				return ErrTimeout("query", "context canceled during iteration")
			default:
			}
...
func (s *BadgerStorage) Delete(ctx context.Context, before time.Time) error {
	...
	err := s.db.View(func(txn *badger.Txn) error {
		...
		for it.Rewind(); it.Valid(); it.Next() {
			...
			select {
			case <-ctx.Done():
				return ErrTimeout("delete", "context canceled during scan")
			default:
			}
		}
	...
	for i := 0; i < len(keysToDelete); i += batchSize {
		...
		select {
		case <-ctx.Done():
			return ErrTimeout("delete", "context canceled during batch delete")
		default:
		}
	}
	...
	for {
		...
		case <-ticker.C:
			...
			*dbSize = totalSize
			...
			if err != nil && !stdErrors.Is(err, badger.ErrNoRewrite) {
