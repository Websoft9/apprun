...
- [x] 集成 `go-i18n` 库 (v2)
- [x] 创建翻译文件（TOML 格式）
- [x] 创建语言元数据文件（`languages.yaml`）
- [x] 实现语言元数据管理（加载、查询、API）
- [x] 实现语言检测中间件
- [x] 实现翻译函数（支持 Context）
- [x] 支持 zh-CN、en-US 两种语言

### 架构要求
- [x] pkg/i18n 包独立运行，无外部依赖
- [x] 翻译文件在启动时从指定路径加载
- [x] 支持翻译缺失的降级处理
- [x] 语言检测失败时使用默认语言
...
- [x] 添加依赖：`go get github.com/nicksnyder/go-i18n/v2 github.com/BurntSushi/toml`
- [x] 创建 `core/locales/` 目录
- [x] 创建 `core/locales/languages.yaml`（语言元数据配置）
- [x] 创建 `core/pkg/i18n/` 目录
- [x] 创建 `core/pkg/middleware/` 目录

### Phase 2: 翻译文件 (45 分钟)
- [x] 创建 `active.zh-CN.toml`（中文翻译）
- [x] 创建 `active.en-US.toml`（英文翻译）
- [x] 添加常用错误和成功消息

### Phase 3: pkg/i18n 核心包 (2 小时)
- [x] 实现 `i18n.go`：Init、Translate、IsSupported 等函数
- [x] 实现 `languages.go`：LoadLanguageMetadata、GetSupportedLanguages 等函数
- [x] 实现 `context.go`：Context 辅助函数
- [x] 编写单元测试

### Phase 4: 语言检测中间件 (1.5 小时)
- [x] 实现 `middleware/language.go`
- [x] 支持 Query、Header、Cookie 检测
- [x] 编写中间件测试

### Phase 5: 集成和文档 (1.5 小时)
- [x] 在 `main.go` 中初始化 i18n
- [x] 在路由中注册中间件
- [x] 编写使用文档和示例
- [x] 运行所有测试
